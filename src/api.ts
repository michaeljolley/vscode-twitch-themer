...
export class API {
  ...
  public static async isTwitchUserFollowing(twitchUserId: string | undefined, state: vscode.Memento) {
    if (twitchUserId) {
      ...
      if (accessToken && currentUserId) {
        ...
        try {
          const res = await fetch(url, {
            method: 'GET',
            headers: { 'Authorization': `Bearer ${accessToken}`, 'client-id': 'ts9wowek7hj9yw0q7gmg27c29i6etn' }
          });
          const { data } = (await res.json()) as { data: any[] };
          return (data.length > 0) ? true : false;
        }
        catch (err: any) {
          Logger.log(LogLevel.debug, `failed to retrieve Twitch user following status: ${err.message}`);
          return false;
        }
      } else {
        Logger.log(LogLevel.debug, "Failed to retrieve Twitch credentials from the user store");
        return false;
      }
    }
  ...
  public static async getUserDetails(token: string | null) {
    ...
    try {
      const res = await fetch(url, {
        method: 'GET',
        headers: { 'Authorization': `Bearer ${token}`, 'client-id': 'ts9wowek7hj9yw0q7gmg27c29i6etn' }
      })
      ...
      Logger.log(LogLevel.error, `Failed to retrieve Twitch user details: ${err.message}`);
      return undefined;
    }
  }
  ...
  public static async isValidExtensionName(extensionName: string): Promise<{ available: boolean; reason?: ThemeNotAvailableReasons; label?: string[] }> {
     ...
     */
     onst repoUrlMatches = body.match(/\"GitHubLink\":\"https:\/\/github.com\/([--:\w?@%&+~#=]+)(?:\.git)?\"/i);
      f(!repoUrlMatches) {
        return { available: fale, reason: ThemeNotAvailableReasons.noRepositoryFound };
      }

      const repoUrl = `https://raw.githubusercontent.com/${repoUrlMatches[1].replace('.git', '')}/master/package.json`;

      s = await axios.get(repoUrl);
      if (res.status === 200) {
      ...
      blic static async getStreamIsActive(_state: vscode.Memento): Promise<boolean> {
    nst accessToken = _state.get(ExtensionKeys.account);
    const currentUserId = _state.get(ExtensionKeys.userId);
    if (accessToken && currentUserId) {
      st url = `https://api.twitch.tv/helix/streams?user_id=${currentUserId}`;
        t res = await axios.get(url, { headers: { 'Authorization': `Bearer ${accessToken}`, 'client-id': 'ts9wowek7hj9yw0q7gmg27c29i6etn' } });
      nst json: any = res.data;
    ...
    return { available: false, reason: ThemeNotAvailableReasons.marketplaceRequestFailed };
  }
}
}
